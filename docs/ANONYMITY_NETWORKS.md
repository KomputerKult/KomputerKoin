# Anonymity Networks with KomputerKult

Currently only Tor and I2P have been integrated into KomputerKult. The usage of
...
is made or until `komputerkoind` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `komputerkoin-wallet-cli` and
`komputerkoin-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `komputerkoind` the ip/port of a socks proxy provided by a
...
which tells `komputerkoind` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `komputerkoind` the inbound address, network
...
which tells `komputerkoind` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `komputerkoind`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `komputerkoind` localhost port 30000.
...
`komputerkoind` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/komputerkoin
...
This will store key information in `/var/lib/tor/data/komputerkoin` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/komputerkoin/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the KomputerKult network should have
similar timestamps.
...
### Intermittent KomputerKult Syncing

If a user only runs `komputerkoind` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `komputerkoind` as often as possible to conceal when transactions are being sent.
...
`komputerkoind` currently selects two outgoing connections every 5 minutes for
...
`komputerkoind` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `komputerkoind` should likely disconnect
