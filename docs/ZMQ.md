# The Current/Future Status of ZMQ in KomputerKult

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. KomputerKult
...
matching supported by both KomputerKult and ZMQ. The `format`, `context` and `event`
...
The KomputerKult daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The KomputerKult daemon also ensures that
...
The KomputerKult daemon will send a `txpool_add` pub exactly once for each
