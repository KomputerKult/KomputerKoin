...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/komputerkoind"
  then
    komputerkoind="$dir/komputerkoind"
    break
  fi
done
if test -z "$komputerkoind"
then
  echo "komputerkoind not found"
  exit 1
fi
echo "Found: $komputerkoind"

TORDIR="$DIR/komputerkoin-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting komputerkoind"
  tail -n 400 "$HOME/.bitkomputerkoin/bitkomputerkoin.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*KomputerKult\ \'" | tail -n 20
  exit 1
fi
