...
Core tests take longer than any other KomputerKult tests, due to the high amount of computational work involved in validating core components.
...
To run only KomputerKult's core tests (after building):
...
To run only KomputerKult's crypto tests (after building):
...
```bash
komputerkoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `komputerkoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only KomputerKult's hash tests (after building):
...
To run only KomputerKult's performance tests (after building):
...
To run only KomputerKult's unit tests (after building):
