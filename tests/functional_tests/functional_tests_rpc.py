...
# a main offline komputerkoind, does most of the tests
# a restricted RPC komputerkoind setup with RPC payment
# two local online komputerkoinds connected to each other
N_MONERODS = 4

# 4 wallets connected to the main offline komputerkoind
# a wallet connected to the first local online komputerkoind
N_WALLETS = 5

WALLET_DIRECTORY = builddir + "/functional-tests-directory"
DIFFICULTY = 10

komputerkoind_base = [builddir + "/bin/komputerkoind", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "komputerkoind_p2p_port", "--rpc-bind-port", "komputerkoind_rpc_port", "--zmq-rpc-bind-port", "komputerkoind_zmq_port", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "komputerkoind_data_dir", "--log-level", "1"]
komputerkoind_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/komputerkoin-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_MONERODS):
  command_lines.append([str(18180+i) if x == "komputerkoind_rpc_port" else str(18280+i) if x == "komputerkoind_p2p_port" else str(18380+i) if x == "komputerkoind_zmq_port" else builddir + "/functional-tests-directory/komputerkoind" + str(i) if x == "komputerkoind_data_dir" else x for x in komputerkoind_base])
  if i < len(komputerkoind_extra):
    command_lines[-1] += komputerkoind_extra[i]
  outputs.append(open(builddir + '/tests/functional_tests/komputerkoind' + str(i) + '.log', 'a+'))
